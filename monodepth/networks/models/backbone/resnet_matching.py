...
from monodepth.networks.utils.monodepth_utils import BackprojectDepth, Project3D
from vision_base.networks.models.backbone.resnet import resnet
...
class ResnetEncoderMatching(nn.Module):
    ...
    def __init__(self, depth, pretrained, input_height, input_width,
                 min_depth_bin=0.1, max_depth_bin=20.0, num_depth_bins=96,
                 adaptive_bins=False, depth_binning='linear', **kwargs):
        ...
        encoder = resnet(depth, pretrained=pretrained, **kwargs)
    ...
    def compute_depth_bins(self, min_depth_bin, max_depth_bin):
        ...
        self.depth_bins = torch.from_numpy(self.depth_bins).float().cuda()

        self.warp_depths = []
        for depth in self.depth_bins:
            depth = depth.new_ones((1, self.matching_height, self.matching_width)) * depth
            self.warp_depths.append(depth)
        self.warp_depths = torch.stack(self.warp_depths, 0).float().cuda()


    def match_features(self, current_feats, lookup_feats, relative_poses, P2):
        ...
        batch = current_feats.shape[0]

        K = np.zeros([batch, 4, 4])
        K[:, 0:3, 0:3] = P2[:, 0:3, 0:3].cpu().numpy()
        K[:, 3, 3] = 1
        inv_K = np.linalg.pinv(K)

        K = current_feats.new(K)
        inv_K = current_feats.new(inv_K)
        ...
        for batch_idx in range(len(current_feats)):
            ...
            _invK = inv_K[batch_idx:batch_idx + 1]
            ...
            for lookup_idx in range(_lookup_feats.shape[1]):
                ...
                b, _, h, w = self.warp_depths.shape
                # ignore missing images
                if lookup_pose.sum() == 0:
                    continue

                lookup_feat = lookup_feat.repeat([self.num_depth_bins, 1, 1, 1])
                pix_locs = self.projector(world_points, _K, lookup_pose, b, h, w)
    ...
    def forward(self, current_image, lookup_images, poses, P2,
        ...
        with torch.no_grad():
            ...
            cost_volume, missing_mask = \
                self.match_features(current_feats, lookup_feats, poses, P2)
